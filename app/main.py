...
def main():
    # App title
    st.set_page_config(layout="wide", page_title="YouTube Video Summary")
    ...
    if not api_key:
        with st.sidebar(key="no_api_key"):
            st.write(
                "You must please provide a valid OpenAI API key in the sidebar to access this app."
            )
        exit()
    ...
    if api_key:
        with st.sidebar:
            ...
            user_selected_model = st.selectbox(
                "Select model",
                options=model_names,
                help=model_information,
                index=best_default_model_index,
            )
            st.markdown(
                "For more information on models and their costs, check the [OpenAI API Models page](https://platform.openai.com/docs/models/gpt-4-and-gpt-4-turbo)"
            )
